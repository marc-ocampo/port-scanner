...
def update_target_hosts_for_ip_range(input_address):
  ip_split = input_address.split('.')
...
def input_is_ip_range(input_address):
  if 3 == input_address.count('.') and 1 == input_address.count('-'):
    update_target_hosts_for_ip_range(input_address)
    return True
...
def input_is_valid_host(input_address):
  try:
    target_hosts.append(socket.gethostbyname(input_address))
    return True
...
def parse_port_list(input_port_list):
  for port in input_port_list.split(','):
...
def check_input_ports(input_port):
  if '' == input_port:
...
def display_open_port(host, open_port_list):
  divider()
  print 'Scan report for ' + host

  if 0 == len(open_port_list):
    print 'Unable to find open ports for ' + host + '.'
  else:
    for port_service_pair in open_port_list:
...
def port_scanner():
  ...
  if check_input_address(target_host) and check_input_ports(target_port):
    ...
    display_total_time(time_start, time_end)
  else:
